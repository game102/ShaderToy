...
static float SOLO;                            // wether we are in solo mode or not    
...
de castRay( ray r ) {
    ...
    if(SOLO<.5) {                                   // we are in marching mode (as in marching matroshkas, not ray marching ;))
        ...
        s = sin(pt*2.5);
        float3x3 leftRight = RotY(s*.2) * RotZ(s*.1);   // used to make the doll lean left and right
...
float3 GroundMat(de o) {
    ...
    float3 r = reflect(o.rd, o.nor);                  // calculate reflect view vector
